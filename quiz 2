...
def pages (page_spec : str, unique :  bool) -> [int]: #result in ascending order
    ...
    for ele in x:
        ...
        if m.group(2) == '-':
            assert int(m.group(1)) <= int(m.group(3))
            if m.group(4) == None:
                y = list(range(int(m.group(1)), int(m.group(3))+1))
            else:
                y = list(range(int(m.group(1)), int(m.group(3))+1, int(m.group(4)))) 
            lst.extend(y)
        if m.group(2) == ':':
            if m.group(4) == None:
                z = list(range(int(m.group(1)), int(m.group(1))+ int(m.group(3)))) 
            else:
                z = list(range(int(m.group(1)), int(m.group(1))+ int(m.group(3)), int(m.group(4)))) 
            lst.extend(z)
    lst = [ int(ele) for ele in lst ]
    if unique == True:
        myset = set(lst)
        returnlst = list(myset)
        return returnlst
    return sorted(lst)


def expand_re(pat_dict:{str:str}):
    for x in pat_dict.keys():    
        for k in pat_dict.keys():
            pat_dict[k] = re.sub('#'+ x +'#', '(?:' + pat_dict[x] + ')', pat_dict[k])
    return
